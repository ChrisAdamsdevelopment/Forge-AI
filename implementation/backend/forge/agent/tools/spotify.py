...
async def spotify_navigate_home() -> dict[str, str]:
    ...
    try:
        ...
        await page.goto(
            "https://open.spotify.com", wait_until="domcontentloaded", timeout=30000
        )
...
async def spotify_search_song(
    song_name: str, artist_name: str | None = None
) -> dict[str, Any]:
    ...
    try:
        ...
        first_result_title = await page.locator(
...
async def spotify_play_first_result() -> dict[str, str]:
    ...
    try:
        ...
        playing_title = await page.locator(
            "div[data-testid='now-playing-widget'] span"
        ).first.text_content()
...
async def spotify_create_playlist(
    playlist_name: str, is_public: bool = False
) -> dict[str, Any]:
...
async def spotify_screenshot() -> dict[str, str]:
    ...
    try:
        ...
        return {"status": "ok", "url": page.url, "screenshot_base64": screenshot_b64}
    except Exception as exc:
        return {"status": "error", "reason": str(exc)}
