...
@mcp.tool(description="Open an application by name or path.", destructiveHint=True)
async def open_app(app_name_or_path: str) -> dict:
    """Open application."""
    return await app_open(app_name_or_path)


@mcp.tool(description="Bring a window to focus by app name.", destructiveHint=True)
async def focus_app(app_name: str) -> dict:
    """Focus application window."""
    return await app_focus(app_name)


@mcp.tool(description="List all open windows and applications.", readOnlyHint=True)
