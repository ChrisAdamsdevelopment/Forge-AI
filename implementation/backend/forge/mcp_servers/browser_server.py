...
@mcp.tool(description="Navigate browser to the provided URL.", annotations={"readOnlyHint": True})
async def browser_nav(url: str) -> dict:
    """Navigate to URL."""
    return await browser_navigate(url)


@mcp.tool(description="Capture a screenshot and return as base64.", annotations={"readOnlyHint": True})
async def browser_snap() -> dict:
    """Take screenshot."""
    return await browser_screenshot()


@mcp.tool(description="Click a page element using CSS selector.", annotations={"destructiveHint": True})
async def browser_btn_click(selector: str) -> dict:
    """Click element."""
    return await browser_click(selector)


@mcp.tool(description="Type text into an input field.", annotations={"destructiveHint": True})
